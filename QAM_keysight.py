...
#         # Rx_X_CMA = Downsample(Rx_X_CMA, downsample=1, downnum=0)
#         # Rx_Y_CMA = Downsample(Rx_Y_CMA, downsample=1, downnum=0)
...
# ----------------------------------------------------------------------------------------------------------------------
    ...
    Histogram2D_Hot("FreqOffsetCompX", Rx_X_FO, path=cur_dir)
    Histogram2D_Hot("FreqOffsetCompY", Rx_Y_FO, path=cur_dir)
...
# ----------------------------------------------------------------------------------------------------------------------
    ...
    Histogram2D_Hot('Rx_corr_X', Rx_corr_X, snr, evm, bercount,path=cur_dir)
    print("Eyepos:", eyepos)
    ...
    Histogram2D_Hot('Rx_corr_Y', Rx_corr_Y, snr, evm, bercount,path=cur_dir)
    print("Eyepos:", eyepos)
    print("BERcount:", bercount, "BER:", ber)
    print("SNR:", snr, "EVM:", evm)
    snry[eyepos] = snr
...
ParaRecord(eyepos_X=np.argmax(snrx), eyepos_Y=np.argmax(snry), cmataps=cma.cmataps, cmastepsize=cma.stepsize, snry=np.max(snry),
           snrx=np.max(snrx), corr_indx=corrindx, datapath=parameter.datapath)
# ----------------------------------------------------------------------------------------------------------------------
#                          NN equalizer
# ----------------------------------------------------------------------------------------------------------------------
"""
...
# ----------------------------------------------------------------------------------------------------------------------
## complex volterra
equalizer_complex = Equalizer(Tx_corr_Y, Rx_corr_Y, 3, [21, 11, 15], 0.12)
...
Histogram2D_Hot("ComplexVolterra_X", Rx_complex_volterra, snr_complex, evm_complex, bercomplex,path=cur_dir)
## real volterra
equalizer_real = Equalizer(np.real(Tx_corr_Y), np.real(Rx_corr_Y), 3, [21, 21, 21], 0.12)
equalizer_imag = Equalizer(np.imag(Tx_corr_Y), np.imag(Rx_corr_Y), 3, [21, 21, 21], 0.12)
...
Histogram2D_Hot("RealVolterra_Y_0.11", Rx_real_volterra, snr_volterra, evm_volterra, bercount,path=cur_dir)

ParaRecord(eyepos_X=np.argmax(snrx), eyepos_Y=np.argmax(snry), cmataps=cma.cmataps, cmastepsize=cma.stepsize, snry=np.max(snry),
           snrx=np.max(snrx), corr_indx=corrindx, datapath=parameter.datapath, volterrataps=equalizer_real.taps, volterraoverhead=equalizer_real.overhead, snr_volterra=snr_volterra)
# ----------------------------------------------------------------------------------------------------------------------
#                           Tektronix DSP QP20
# ----------------------------------------------------------------------------------------------------------------------
