...
record_file = '../OB2B_CFP2_26GBd_QAM_PN15_600mV_resample.xlsx'
cur_dir = '..\image_OB2B_CFP2_26GBd_QAM_resample'
Path(cur_dir).mkdir(parents=True, exist_ok=True)
is_continue = 1
is_volterra = 0
CD_comp = 0
# parameter = Parameter(simulation=True)
parameter = Parameter(proc_file=r'B2B_CFP2_26GBd_qam_PN15_600mV.mat', tektronic=False)
print('SymbolRate={}GBaud'.format(parameter.symbolRate / 1e9), 'Pamorder={}'.format(parameter.pamorder),
          'downsample_number={}'.format(parameter.downsample_number))
# print('Tx Length={}'.format(len(parameter.TxXI)), 'Rx Length={}'.format(len(parameter.RxXI)))
# ----------------------------------------------------------------------------------------------------------------------
#                               Tx to Bit
# ----------------------------------------------------------------------------------------------------------------------
# Tx_Signal_X = np.array(Tx2Bit_simu(parameter.TxXI, parameter.TxXQ, parameter.pamorder))
Tx_Signal_X = np.array(Tx2Bit(parameter.TxXI, parameter.TxXQ, parameter.pamorder))
# Tx_Signal_Y = np.array(Tx2Bit_simu(parameter.TxYI, parameter.TxYQ, parameter.pamorder))
...
# window = signal.windows.kaiser(parameter.resamplenumber, beta=14)
Rx_XI, Rx_XQ = DataNormalize(signal.resample_poly(parameter.RxXI, up=parameter.upsamplenum, down=parameter.downsample_number),
                             signal.resample_poly(parameter.RxXQ, up=parameter.upsamplenum, down=parameter.downsample_number),
                             parameter.pamorder)
Rx_YI, Rx_YQ = DataNormalize(signal.resample_poly(parameter.RxYI, up=parameter.upsamplenum, down=parameter.downsample_number),
                             signal.resample_poly(parameter.RxYQ, up=parameter.upsamplenum, down=parameter.downsample_number),
                             parameter.pamorder)
...
# ----------------------------------------------------------------------------------------------------------------------
snrx, snry = np.zeros((16, 1)), np.zeros((16, 1))
evmx, evmy = np.zeros((16, 1)), np.zeros((16, 1))
#Eye position scan
for eyepos in range(start, end):
    Rx_XI_eye = DataNormalize(signal.resample_poly(Rx_XI[eyepos:], up=1, down=32), [],
                              parameter.pamorder)
    Rx_XQ_eye = DataNormalize(signal.resample_poly(Rx_XQ[eyepos:], up=1, down=32), [],
                              parameter.pamorder)
    Rx_YI_eye = DataNormalize(signal.resample_poly(Rx_YI[eyepos:], up=1, down=32), [],
                              parameter.pamorder)
    Rx_YQ_eye = DataNormalize(signal.resample_poly(Rx_YQ[eyepos:], up=1, down=32), [],
                              parameter.pamorder)
...
# ----------------------------------------------------------------------------------------------------------------------
    if CD_comp:
        CD_comp = CD_compensator(Rx_Signal_X, Rx_Signal_Y, parameter.symbolRate * 2, 0)
        Rx_X_CD, Rx_Y_CD = CD_comp.overlap_save(Rx_Signal_X.size, 4096)
...
# ----------------------------------------------------------------------------------------------------------------------
    for i in range(15, 25, 2):
        if CD_comp:
            cma = CMA(Rx_X_CD, Rx_Y_CD, taps=i, iter=15)
        else:
            cma = CMA(Rx_Signal_X[0:100000], Rx_Signal_Y[0:100000], taps=i, iter=15)
        cma.CMA_butterfly()
...
#                               16QAM CMA
# ----------------------------------------------------------------------------------------------------------------------
        for tap in range(11, 21, 2):
            # cma = CMA(Rx_Signal_X[0:300000], Rx_Signal_Y[0:300000], taps=i, iter=60)
            cma = CMA(Rx_X_CMA, Rx_Y_CMA, taps=tap, iter=15)
            cma.RDE_butterfly()
            Rx_X_CMA, Rx_Y_CMA = cma.rx_x_single, cma.rx_y_single
            print('CMA Batch Size={}'.format(cma.batchsize), 'CMA Stepsize={}'.format(cma.stepsize),
                'CMA OverHead={}%'.format(cma.overhead * 100))
            print("CMA COST={}".format(cma.costfunx))
            Histogram2D_Hot('CMAX_{}(taps{})'.format(eyepos, tap), Rx_X_CMA, path=cur_dir)
            Histogram2D_Hot('CMAY_{}(taps{})'.format(eyepos, tap), Rx_Y_CMA, path=cur_dir)
if is_continue:
# ----------------------------------------------------------------------------------------------------------------------
#                               FFT-FOE
# ----------------------------------------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------------------------------------
    ...
    ## complex volterra
    if is_volterra:
        equalizer_complex = Equalizer(Tx_corr_X[0:70000], Rx_corr_X[0:70000], 3, [21, 21, 25], 0.3)
        ...
        ## real volterra
        equalizer_real = Equalizer(np.real(Tx_corr_X[0:70000]), np.real(Rx_corr_X[0:70000]), 3, [21, 21, 21], 0.32)
        equalizer_imag = Equalizer(np.imag(Tx_corr_X[0:70000]), np.imag(Rx_corr_X[0:70000]), 3, [21, 21, 21], 0.32)
