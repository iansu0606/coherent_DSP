    ...
    def qam_4_side_conj_SBD_polarization(self):          #A FAMILY OF ALGORITHMS FOR BLIND EQUALIZATION OF QAM SIGNALS
        ...
        for it in range(self.iterator):
            for indx in range(self.center, self.datalength - self.center):
                ...
                # epsilon_real = epsilon_lambda * epsilon_real + (1 - epsilon_lambda)*(abs(Decision[np.argmin(abs(HardDecision_X_real))]-np.real(exout[indx])))
                # p_real = 7.1467 * (1 - np.exp(8 * epsilon_real - 0.24)) / (1 + np.exp(8 * epsilon_real - 0.24)) + 9.1467
                # gamma_real = 2 ** (-p_real)
                # epsilon_imag = epsilon_lambda * epsilon_imag + (1 - epsilon_lambda)*(abs(Decision[np.argmin(abs(HardDecision_X_imag))]-np.imag(exout[indx])))
                ...
                #
                #
                # # if (Decision[np.argmin(abs(HardDecision_X_real))] == 7) or (Decision[np.argmin(abs(HardDecision_X_real))] == -7):
                # # HardDecision_X_real = np.delete(HardDecision_X_real , np.argmin(abs(HardDecision_X_real)))
                # # errx = errx + gamma_real * np.abs(Decision[np.argmin(abs(HardDecision_X_real))]) * (Decision[np.argmin(abs(HardDecision_X_real))] - np.real(exout[indx]))
                # # HardDecision_X_imag = np.delete(HardDecision_X_imag , np.argmin(abs(HardDecision_X_imag)))
                # # errx = errx + gamma_imag * 1j*(np.abs(Decision[np.argmin(abs(HardDecision_X_imag))]) * (Decision[np.argmin(abs(HardDecision_X_imag))] - np.imag(exout[indx])))
                #
                # # if it < 5:
                # if (Decision[np.argmin(abs(HardDecision_X_real))] == -3):
                ...
                # elif (Decision[np.argmin(abs(HardDecision_X_real))] == 3) :
                #     errx += gamma_real * (np.abs(Decision[np.argmin(abs(HardDecision_X_real)) - 1]) * (Decision[np.argmin(abs(HardDecision_X_real)) - 1] - np.real(exout[indx])))
                # else :
                #     errx += gamma_real * (np.abs(Decision[np.argmin(abs(HardDecision_X_real)) - 1]) * (Decision[np.argmin(abs(HardDecision_X_real)) - 1] - np.real(exout[indx])) + \
                ...
                #
                #
                # if (Decision[np.argmin(abs(HardDecision_X_imag))] == -3):
                ...
                # elif (Decision[np.argmin(abs(HardDecision_X_imag))] == 3) :
                #     errx += gamma_imag * (np.abs(Decision[np.argmin(abs(HardDecision_X_imag)) - 1]) * (Decision[np.argmin(abs(HardDecision_X_imag)) - 1] - np.imag(exout[indx]))) * 1j
                # else :
                ...
                cost_x[indx] = (np.real(exout[indx]) - Decision[np.argmin(abs(HardDecision_X_real))]) ** 2 + 1j * (np.imag(exout[indx]) - Decision[np.argmin(abs(HardDecision_X_imag))]) ** 2
        ...
        for it in range(self.iterator):
            for indx in range(self.center, self.datalength - self.center):
                ...
                if self.mean == True:
                    ...
                    #     np.imag(iny))
                    squ_R_x = np.mean(abs(inx) ** 4) / np.mean(abs(inx) ** 2)
                    squ_R_y = np.mean(abs(iny) ** 4) / np.mean(abs(iny) ** 2)
    ...
    def MCMA_MDD_(self):
        ...
        for it in range(self.iterator):
            for indx in range(self.center, self.datalength - self.center):
                ...
                errx_mcma[indx] = (squ_Rx - np.abs(exout_mcma[indx]) ** 2)
                erry_mcma[indx] = (squ_Ry - np.abs(eyout_mcma[indx]) ** 2)

                hxx_mcma = hxx_mcma + self.stepsize * np.conj(exout_mcma[indx] * errx_mcma[indx]) * (
                    inputrx[indx - self.center:indx + self.center + 1])
                hyy_mcma = hyy_mcma + self.stepsize * np.conj(eyout_mcma[indx] * erry_mcma[indx]) * (
                    inputry[indx - self.center:indx + self.center + 1])
